 ...
 * @description
 **/
public class DBHintShardAlgo implements HintShardingAlgorithm<Integer> {

    private static final String connetWord = "_";
    ...
    public Collection<String> doSharding(Collection<String> collection, HintShardingValue<Integer> hintShardingValue) {
        ...
        for (String each : collection) {
            for (Integer value : hintShardingValue.getValues()) {
                String realTableName = ShardConstants.logicDb + connetWord + value;
                if (each.equals(realTableName)) {
                    result.add(each);
                }
            }
        }
        //如果存在未匹配成功的情况，那么直接返回所有的库
        if(result.size() == 0){
            return collection;
        }
        return result;
